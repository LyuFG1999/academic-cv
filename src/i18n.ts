...
export function isLanguage(value: string | undefined): value is Language { return value === 'zh' || value === 'en' }
export function localizedPath(lang: Language, path = '') { const normalized = path === '/' ? '' : path.replace(/^\//, ''); return `/${lang}/${normalized}`.replace(/\/$/, '') || `/${lang}` }
